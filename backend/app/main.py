...
@app.get("/fighters/{fighter_id}", response_model=schemas.FighterResponse)
...
@app.get("/fighters/", response_model=List[schemas.FighterResponse])
def read_fighters(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    fighters = crud.get_fighters(db, skip=skip, limit=limit)
    return fighters

@app.get("/fights/{fight_id}", response_model=schemas.FightResponse)
...
@app.get("/fights/", response_model=List[schemas.FightResponse])
